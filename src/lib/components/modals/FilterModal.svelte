<script lang="ts">
	...
	$: modalTitle = $currentIdx === 0 ? 'Modpack Filters' : 'Resource Pack Filters';
...
<ComposedModal bind:open={$isModalOpen} on:submit={() => isModalOpen.set(false)}>
					...
					<Column>
						...
						>
							{#each versions as version}
								<SelectItem value={version.id} text={`${version.id} (${version.type})`} />
							{/each}
						</Select>
					</Column>
				</Row>
