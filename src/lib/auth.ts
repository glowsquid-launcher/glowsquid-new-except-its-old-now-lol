...
export const addAccount = (authData: AuthData) =>
  internalAuthState.update((state) => ({
    ...state,
    authAccs: [...state.authAccs, authData]
  }));

export const selectAccount = (idx: number) =>
  internalAuthState.update((state) => ({
    ...state,
    selectedAccountIdx: idx
  }));

export const deleteAccount = (idx: number) =>
  internalAuthState.update((state) => ({
    ...state,
...
export const authState = derived(internalAuthState, ($state) => ({
  ...$state,
  ...
  selectedAccount:
    $state.selectedAccountIdx === null ? null : $state.authAccs[$state.selectedAccountIdx]
}));
